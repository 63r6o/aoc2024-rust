...
fn main() {
    ...
    let mut lab_map = input.lines().enumerate().map(|(row, line)| {
    ...
    fn patrol(positions: &mut HashSet<(usize, usize)>, current_row: usize, current_col: usize, direction: Direction, lab_map: &mut Vec<Vec<char>>) -> usize {
        ...
        let new_direction = if !obstacle_ahead { direction } else {
            match direction {
                Direction::Up => {
    ...
    let part_one = patrol(&mut positions, start_row, start_col, Direction::Up, &mut lab_map);

    println!("part one: {}", part_one);


    fn is_loop_patrol(positions: &mut HashSet<(usize, usize, &str)>, current_row: usize, current_col: usize, direction: Direction, lab_map: &Vec<Vec<char>>) -> i32 {
    ...
    let part_two = positions.iter().fold(0, |res, (row, col)| {
        ...
        res + is_loop_patrol(&mut positions, start_row, start_col, Direction::Up, &obstacled_map)
    });
