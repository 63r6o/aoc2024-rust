...
fn main() {
    ...
    let dijstra_like_stuff = |(x_a, y_a): (i64, i64),
                              (x_b, y_b): (i64, i64),
                              (x_prize, y_prize): (i64, i64)|
     -> (i64, i64) {
        ...
        while !min_heap.is_empty() {
            ...
            if seen.contains(&current_position.coords)
                || current_position.a_steps > 100
                || current_position.b_steps > 100
            {
                continue;
            }
            ...
            if current_position.coords.0 + x_a <= x_prize
                && current_position.coords.1 + y_a <= y_prize
            {
                min_heap.push(Position {
                    used_tokens: current_position.used_tokens + 3,
                    coords: (
                        current_position.coords.0 + x_a,
                        current_position.coords.1 + y_a,
                    ),
            ...
            if current_position.coords.0 + x_b <= x_prize
                && current_position.coords.1 + y_b <= y_prize
            {
                min_heap.push(Position {
                    used_tokens: current_position.used_tokens + 1,
                    coords: (
                        current_position.coords.0 + x_b,
                        current_position.coords.1 + y_b,
                    ),
    ...
    let solve_it_with_maths = |(x_a, y_a): (i64, i64),
                               (x_b, y_b): (i64, i64),
                               (mut x_prize, mut y_prize): (i64, i64)|
     -> (i64, i64) {
    ...
    claws.iter().for_each(|coords| {
        ...
        let (a_steps_two, b_steps_two) = solve_it_with_maths(
            coords[0],
            coords[1],
            (coords[2].0 as i64, coords[2].1 as i64),
        );
